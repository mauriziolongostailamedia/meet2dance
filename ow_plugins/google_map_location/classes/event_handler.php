            ...
            if ( !empty($location['json']) )
            {
                ...
                $data = '<div class="ow_googlemap_location_view_presentation">
                            <div class="ow_googlemap_location_address" >
                                <a class="map-hint-target" data-location=\''.htmlspecialchars(json_encode($location), ENT_QUOTES).'\' href="javascript://">' . $location['address'] . '</a><span class="googlemap_pin ic_googlemap_pin"></span>
                            </div>
...
// -- search --
    ...
    function addJsLib( OW_Event $e )
    {
        if ( !$this->jsLibAdded )
        {
            ...
            $staticUrl = OW::getPluginManager()->getPlugin("googlelocation")->getStaticUrl();
            ...
            switch( GOOGLELOCATION_BOL_LocationService::getInstance()->getMapProvider() )
            {
                case GOOGLELOCATION_BOL_LocationService::PROVIDER_GOOGLE:
                    ...
                    // GOOGLE MAPS API
                    OW::getDocument()->addScript($protocol.'maps.google.com/maps/api/js?' . $key . '&language=' . $languageCode);
                    ...
                    // MAP
                    OW::getDocument()->addScript(OW::getPluginManager()->getPlugin('googlelocation')->getStaticJsUrl().'google/map.js', "text/javascript", GOOGLELOCATION_BOL_LocationService::JQUERY_LOAD_PRIORITY + 1);
                ...
                case GOOGLELOCATION_BOL_LocationService::PROVIDER_BING:
                    ...
                    // BING MAPS API
                    OW::getDocument()->addScript($protocol.'ecn.dev.virtualearth.net/mapcontrol/mapcontrol.ashx?v=7.0'.( $isSSL ? '&s=1' : '' ).'&mkt=' . $languageCode);

                    // MAP
                    OW::getDocument()->addScript(OW::getPluginManager()->getPlugin('googlelocation')->getStaticJsUrl().'bing/map.js', "text/javascript", GOOGLELOCATION_BOL_LocationService::JQUERY_LOAD_PRIORITY + 1);
                    OW::getDocument()->addOnloadScript(" GOOGLELOCATION.credentials = ".json_encode($credentials).";");

                    break;
            }

            //OW::getDocument()->addScript(OW::getPluginManager()->getPlugin('googlelocation')->getStaticJsUrl() . 'google_map.js', "text/javascript", GOOGLELOCATION_BOL_LocationService::JQUERY_LOAD_PRIORITY + 1);
