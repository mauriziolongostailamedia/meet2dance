...
class GoogleLocationConfigForm extends Form
{
    ...
    const SESSION_STAMP = 'location_settings_update_stamp';
    /**
     * Class constructor
     *
     */
    ...
    public function updateValues()
    {
        ...
        OW::getSession()->set(self::SESSION_STAMP, time());
    }
    
    public function getData()
    {
        $time = OW::getSession()->get(self::SESSION_STAMP);
        $data = OW::getSession()->get(self::SESSION_DATA);
        if ( time() - $time > 5 ) 
        {
            OW::getSession()->set(self::SESSION_DATA, array());
            OW::getSession()->set(self::SESSION_STAMP, 0);
            return array();
        }
